...
# Increase recursion limit for Python itself
sys.setrecursionlimit(10000)

# Configuration
OLLAMA_MODEL = "llama3.2"  # Or other local model
...
class AgentState(TypedDict):
    ...
    context: List[str]
    thoughts: List[str]
    planning_steps: List[str]  # Renamed from 'plan' to avoid conflict
    current_step: int
    response: str
...
class TaskPlanner:
    ...
    def run(self, state: AgentState) -> AgentState:
        ...
        state["iteration_count"] = 0  # Initialize iteration counter
        return state
...
class ResponseGenerator:
    ...
    def run(self, state: AgentState) -> AgentState:
        ...
        # Explicitly increment step counter
...
def create_agent_graph(doc_processor):
    ...
    # Create graph without recursion_limit parameter (for older LangGraph versions)
    workflow = StateGraph(AgentState)
    
    # Add nodes - renamed 'plan' to 'planning' to avoid conflict
    ...
    if user_input and st.session_state.agent:
        ...
        with st.spinner("Thinking..."):
            initial_state = AgentState(
                ...
                context=[],
                thoughts=[],
                planning_steps=[],  # Renamed from 'plan'
                current_step=0,
                response="",
                query_refinements=[],
                evaluation="",
            ...
            # Run agent with recursion_limit in config (for older LangGraph versions)
            result = st.session_state.agent.invoke(
                initial_state,
                {"recursion_limit": 50}  # Pass recursion limit in config
            )
